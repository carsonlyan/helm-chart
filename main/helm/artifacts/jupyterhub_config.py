...
OIDC = f"{getenv('OIDC_URL')}/protocol/openid-connect"
c.LocalGenericOAuthenticator.client_id = getenv('OIDC_CLIENT_ID')
c.LocalGenericOAuthenticator.client_secret = getenv('OIDC_CLIENT_SECRET')
c.LocalGenericOAuthenticator.token_url = OIDC + '/token'
c.LocalGenericOAuthenticator.userdata_url = OIDC + '/userinfo'
...
# https://gist.github.com/minrk/924ef40af06cae06116c1c7b60b963e3
...
def my_hook(spawner):
    username = spawner.user.name
    check_call(['/home/root/user-backup.sh'])

    if len(listdir(f'/home/{username}')) < 5:
        check_call(['su', username, '-c', '/home/root/user-init.sh'])
